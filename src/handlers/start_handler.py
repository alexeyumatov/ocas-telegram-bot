...
from src.database.db import start_db, update_username, update_currency, get_username, get_currency
...
class Registration(StatesGroup):
    name_input = State()
    currency_input_callback = State()
    currency_input_by_hand = State()


@router.message(Command("start"), flags=flags)
async def start_cmd(message: Message):
    await start_db(message.from_user.id)
    if await get_currency(message.from_user.id) == None:
        await message.answer(text="<b>Добро пожаловать!</b>\nЯ бот, который может переводить валюты",
                            reply_markup=reg_kb)
    else:
        await message.answer(text=f"Здравствуйте, <b>{await get_username(message.from_user.id)}</b>!\nВы в главном меню.",
...
async def name_input(message: Message, state: FSMContext):
    await message.answer(text=f"Отлично, <b>{message.text}</b>!\n\n"
                         "Теперь выберите предпочитаемую валюту для перевода (ее можно будет сменить в настройках)\n",
                         reply_markup=currency_kb)
    await update_username(message.from_user.id, message.text)
    await state.set_state(Registration.currency_input_callback)
...
@router.callback_query(Registration.currency_input_callback, Text(startswith="curr_", ignore_case=True))
async def currency_input_callback(callback: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    if currency == 'byhand':
        await bot.edit_message_text(text="Примеры формата ввода: <i>RUB</i>, <i>USD</i>, <i>EUR</i>\n\n"
                                    ...
                                    chat_id=callback.from_user.id,
                                    message_id=callback.message.message_id)
        await state.set_state(Registration.currency_input_by_hand)
    else:
...
@router.message(Registration.currency_input_by_hand, F.text, CurrencyInputCheck(), flags=flags)
...
@router.message(Registration.currency_input_by_hand)
